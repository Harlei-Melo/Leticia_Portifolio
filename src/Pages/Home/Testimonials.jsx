...
export default function Testimonial() {
  // Array fixo para substituir os nomes vindos do JSON
  const customNames = ["Harlei Melo", "Amanda Souza", "Maria Gabriella"];

  return (
    <section className="testimonial--section" id="testimonial">
      <div className="portfolio--container-box">
        <div className="portfolio--container">
          <p className="sub--title">Avaliação</p>
          <h2 className="sections--heading">Feedback dos Clientes</h2>
        </div>
      </div>
      <div className="portfolio--section--container">
        {data?.testimonial?.map((item, index) => (
          <div key={item.id} className="testimonial--section--card">
            ...
            {/* Apenas o nome (sem imagens e designations) */}
            <div className="testimonial--section--card--author--detail">
              <p className="text-md testimonial--author--name">
                {customNames[index] /* Exemplo de substituição de nomes */}
              </p>
            </div>
          </div>
        ))}
